setwd("~/PycharmProjects/Nostromo/success_rate_plots_code_and_data")  
library(ggplot2)
  library(reshape2)
  library(ggpubr)
  library(extrafont)
  ...
  ss_aa$Quality = factor(ss_aa$Quality, levels=c("Medium", "Acceptable", "Near-Acceptable", "Low"))
  ...
  ss_aa_plot = ggplot(ss_aa, aes(cutoff, complex, fill=ss_aa$Quality)) +
    ...
    scale_fill_manual(values=c("#b2df8a", "#a6cee3", "light gray", "white")) +
    theme(
  ...
  ss_cg$Quality = factor(ss_cg$Quality, levels=c("Medium", "Acceptable", "Near-Acceptable", "Low"))
 ...
 aa_plot = annotate_figure(aa_plot, top = text_grob("All-Atom \n", size = 20))
  ...
  postscript("legend.eps", fonts=c("Courier"))
  p <- recordPlot()
  legtext <- c("High", "Medium", "Acceptable","Near-Acceptable", "Low")
  plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
  legend(x="bottomleft", bty = "n", x.intersp=0.5, horiz = T,
         legend=legtext, 
         fill=c("#33a02c", "#b2df8a", "#a6cee3", "light gray", "white"), 
         cex = 1,
         text.width=c(0, 0.045, 0.058, 0.071, 0.088))
  p
  dev.off()
  ...
  ggsave("success-rate.png", complex_plot, width=12, height=13)

...
bound_l = [p for p in glob.glob('*prot*pdb') if not 'cg' in p]

# remove redundancies
pdb_dic = dict([(string.uppercase[int(p[-5])-1], p) for p in bound_l])

# pdb_dic = {'A': 'bound_protA.pdb', 'B': 'bound_protB.pdb'}
# add chain
...
if not os.path.isfile('dna_restraints.def'):
	print 'WARNING: dna_restraints.def not generated, is there a DNA/RNA molecule? Is it a double strand? check aa2cg conversion'

if not os.path.isfile('dna-aa_groups.dat'):
	print 'WARNING: dna-aa_groups.dat not generated, this is needed for DNA/RNA restrictions, check aa2cg conversion'
